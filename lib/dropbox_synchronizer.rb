module DiscourseBackupToDropbox
  class DropboxSynchronizer < Synchronizer #inherit from the base class
    ...
    def perform_sync(dropbox_backup_files)
      ...
      file_difference_remote.dbx.delete("/#{folder_name}/#{filename}") # passed @dbx as a second argument here, dont know if that works
    end                                                                  # and took out the loop

    def upload(dbx, folder_name, file_name, full_path, size)
      if size < UPLOAD_MAX_SIZE then
        @dbx.upload("/#{folder_name}/#{file_name}", File.open(full_path, "r"))
      else
        backup.chunked_upload(@dbx, folder_name, file_name, full_path)
      end
    end

    def chunked_upload(dbx, folder_name, file_name, full_path)
